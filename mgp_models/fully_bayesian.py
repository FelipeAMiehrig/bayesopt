...
from mgp_models.posterior import WeightedGMPosterior
_sqrt5 = math.sqrt(5)
...
def exp_kernel(X: Tensor, lengthscale: Tensor) -> Tensor:
    """Matern-5/2 kernel."""
    dist = compute_dists(X=X, lengthscale=lengthscale)
...
class MGPPyroModel(PyroModel):
    ...
    def sample(self) -> None:
        ...
        K = exp_kernel(X=self.train_X, lengthscale=lengthscale)
    ...
    def sample_prior(self) -> Dict[str, Tensor]:
        r"""Sample from the SAAS model.
        This samples the mean, noise variance, outputscale, and lengthscales according
        to the SAAS prior.
        """
        ...
        noise = self.sample_noise(**tkwargs)
        lengthscale = self.sample_lengthscale(dim=self.ard_num_dims, **tkwargs)
        return {"outputscale":outputscale.unsqueeze(0),
...
class MGPFullyBayesianSingleTaskGP(ExactGP, BatchedMultiOutputGPyTorchModel):
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any], strict: bool = True):
        ...
        if not isinstance(self.pyro_model, MGPPyroModel):
    ...
    def posterior(
        ...
        **kwargs: Any,
        ...
        posterior = WeightedGMPosterior(distribution=posterior.distribution, ll=ll)
        return posterior
